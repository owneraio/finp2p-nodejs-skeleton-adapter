...
const wrappedResponse = (methodName: string, opMetadata: OperationMetadata | undefined, args: [cid: string] | [cid: string, errorCode: number, errorMessage: string]): OperationStatus => {
  ...
  switch (methodName) {
    ...
    case compiletimeMethodName<TokenService>('swapSingle'):
      return pendingOrError(cid => pendingSwapSingleOperation(cid, opMetadata), (cid, code, message) => failedSwapSingleOperation(code, message));
