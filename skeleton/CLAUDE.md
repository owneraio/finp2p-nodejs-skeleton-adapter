...
- **`whitelist/service.ts`** &mdash; `InvestorWhitelistServiceImpl` &mdash; which parties may transact which asset, keyed by `(party, assetId)`, plus an arbitrary adapter-defined `config` stored verbatim as jsonb and never interpreted by the skeleton. Re-whitelisting the same pair **replaces** the config (unlike network-account onboarding, where a repeat create replays the existing binding &mdash; whitelisting is configuration, not an identity binding). Dewhitelisting something absent is a success so operator retries don't fail. Optional `InvestorWhitelistValidator` may reject (throw `ValidationError`) or normalize the config; it lives on the **service**, not the route config, so direct callers are guarded too.

  **This store-backed implementation is optional, and often wrong.** When the ledger enforces membership itself, implement `InvestorWhitelistService` directly against it &mdash; resolve the party to an address, delegate to the token standard, and let `isWhitelisted` read on-chain enforcement. Do **not** run both: any flow that mutates the ledger directly leaves these rows stale, so pick one owner of the truth. A ledger-backed implementation may also be unable to report the `config` originally submitted from `getWhitelist`, since it can only report what the ledger knows.
...
7. For investor whitelisting, pass `options.whitelistService` &mdash; either your own implementation over ledger enforcement, or `InvestorWhitelistServiceImpl(store, validator?)` when the state genuinely lives off-ledger. Omit it and the endpoints aren't mounted. Pass `options.whitelistOptions.authToken` to guard them.
...
- `20260810090000_create_investor_whitelist_table.sql` &mdash; `investor_whitelist` table: primary key (party_type, party_id, asset_id), arbitrary `config` jsonb
