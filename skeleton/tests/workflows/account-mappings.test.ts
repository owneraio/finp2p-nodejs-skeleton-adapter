...
import { PgAccountMappingStore } from '../../src/storage'
import { Pool } from 'pg'
...
describe("account mappings", () => {
  ...
  let pool: Pool;
  let store: PgAccountMappingStore;
  ...
  beforeEach(async () => {
    ...
    store = new PgAccountMappingStore(pool);
  })
  ...
  test("duplicate insert is idempotent", async () => {
    await store.saveOwnerMapping("fin-1", { ledgerAccountId: "0xAAA" });
    await store.saveOwnerMapping("fin-1", { ledgerAccountId: "0xAAA" });

    const all = await store.getOwnerMappings();
    expect(all).toHaveLength(1);
    expect(all[0].fields.ledgerAccountId).toBe("0xaaa");
  });

  test("update existing field value", async () => {
    await store.saveOwnerMapping("fin-1", { ledgerAccountId: "0xOLD" });
    await store.saveOwnerMapping("fin-1", { ledgerAccountId: "0xNEW" });
  ...
  test("retrieve mappings by field value (case-insensitive)", async () => {
    await store.saveOwnerMapping("fin-1", { ledgerAccountId: "0xAbCdEf" });
    await store.saveOwnerMapping("fin-2", { ledgerAccountId: "0xABCDEF" });
  ...
  test("delete specific field", async () => {
    ...
    await store.deleteOwnerMapping("fin-1", "ledgerAccountId");
  ...
  test("delete all mappings for finId", async () => {
    ...
    await store.deleteOwnerMapping("fin-1");
  ...
  test("list all mappings", async () => {
    await store.saveOwnerMapping("fin-1", { ledgerAccountId: "0xAAA" });
    await store.saveOwnerMapping("fin-2", { ledgerAccountId: "0xBBB" });
    await store.saveOwnerMapping("fin-3", { ledgerAccountId: "0xCCC" });
  ...
  test("empty result for nonexistent finId", async () => {
    const mappings = await store.getOwnerMappings(["does-not-exist"]);
    expect(mappings).toHaveLength(0);
  });
  ...
  test("getByFieldValue returns all fields for matched finIds", async () => {
    ...
    await store.saveOwnerMapping("fin-2", {
      ledgerAccountId: "0xshared",
