...
import {EIP712_DOMAIN, hashEIP712, signEIP712WithPrivateKey} from "../../helpers";
...
export class ProofProvider {
  ...
  constructor(logger: winston.Logger, finP2PClient: FinP2PClient, signerPrivateKey: string) {
    this.logger = logger;
    this.finP2PClient = finP2PClient;
    this.signerPrivateKey = signerPrivateKey;
  }

  private async ledgerProof(receipt: Receipt): Promise<Receipt> {
    ...
    switch (policy.type) {
      ...
      case "SignatureProofPolicy":
        ...
        if (policyDomain !== null) {
          this.logger.info("Using domain from asset metadata: ", policyDomain);
        }
        ...
        this.logger.info("Signing receipt with EIP712", {primaryType, types, message});
        ...
        this.logger.info("Receipt signed", {hash, signature});
